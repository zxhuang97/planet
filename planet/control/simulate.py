...
def collect_rollouts(
    ...
    if agent_config.agent=='dual1':
        agent = mpc_agent.dualMPCAgent1(batch_env, step, False, False, agent_config)
    elif agent_config.agent=='dual2':
        agent = mpc_agent.dualMPCAgent2(batch_env, step, False, False, agent_config)
    else:
...
def simulate_step(batch_env, algo, log=True, reset=False):
    ...
    def _define_step():
        ...
        # a = tf.print('leng ', inc_length)
        with tf.control_dependencies([add_score, inc_length]):
            agent_indices = tf.range(len(batch_env))
            # step = tf.print('why bro')
            step = algo.experience(
                agent_indices,
                batch_env.action,
                batch_env.done,
                prevob,
                step_summary)
            # summary = tf.summary.merge([experience_summary])
            # summary = tf.constant('')
        # with tf.control_dependencies([summary]):
        return step, add_score, inc_length
    ...
    with tf.name_scope('simulate'):
        ...
        with tf.control_dependencies([end_episode]):
            summary = tf.summary.merge([
                _define_summaries(), begin_episode, end_episode])
        with tf.control_dependencies([summary, step]):
            score = 0.0 + score
            done = batch_env.done
