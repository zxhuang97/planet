...
def define_summaries(graph, config, cleanups):
    ...
    heads.unlock()
    if config.aug == 'rad' and not config.aug_same:
        heads['image'] = lambda features: transform(graph.heads['image'](tf.concat([features,
                                  tf.constant(3., dtype=tf.float32, shape=tuple(features.shape[:2])+(2,))], -1)))
    else:
    ...
    with tf.variable_scope('general'):
        ...
        summaries.append(tf.summary.scalar('cstr_pct', graph.cstr_pct))
    ...
    # p5 = tf.print('Check pts 5', None)
    # with tf.control_dependencies([p5]):
    ...
    # p6 = tf.print('Check pts 6', None)
    # with tf.control_dependencies([p6]):
    ...
    # p7 = tf.print('Check pts 7', None)
    # with tf.control_dependencies([p7]):
    ...
    a = tf.print('num of trajectories ', sim_return)
    with tf.control_dependencies([a]):
        score = tf.identity(score)
    summaries = tf.summary.merge(summaries)

    # score=tf.constant(0.0)
    # with tf.control_dependencies([score,summaries]):
    #     p71 = tf.print('Check pts 71', None)
    # with tf.control_dependencies([p71]):
    # score = tf.identity(score)
