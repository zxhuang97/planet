    ...
    for name, scale in config.loss_scales.items():
        ...
        if name == 'divergence':
            ...
            if config.contra_unit == 'traj':
                ...
                contra_loss, cstr_pct = contra_traj_lossV6(pred, target[name], horizon=config.contra_horizon)
                ...
                contra_loss, cstr_pct = contra_traj_lossV7(
                                                    pred, target[name], horizon=config.contra_horizon, temp=config.temp)
            elif config.contra_unit == 'simclr':
                print('Using simclr trajectory loss ', config.contra_horizon)
                contra_loss, cstr_pct = contra_traj_lossV8(pred, target[name], horizon=config.contra_horizon)
