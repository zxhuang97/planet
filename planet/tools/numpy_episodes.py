...
def numpy_episodes(
        ...
        num_chunks=None, preprocess_fn=None, aug_fn=None, simclr=False):
    ...
    def sequence_preprocess_fn(sequence):
        if preprocess_fn:
            sequence['image'], noise = preprocess_fn(sequence['image'], return_noise=True)
            if 'ori_img' in sequence.keys():
                sequence['ori_img'] = preprocess_fn(sequence['ori_img'], noise=noise)
        if simclr:
            print('ccc ', sequence)
            sequence['image'] = tf.reshape(sequence['image'], (shape[0], shape[1]) + tuple(sequence['image'].shape[2:]))
            sequence['action'] = tf.reshape(sequence['action'], (shape[0], shape[1], sequence['action'].shape[2]))
            sequence['reward'] = tf.reshape(sequence['reward'], (shape[0], shape[1]))
            sequence['aug'] = tf.reshape(sequence['aug'], (shape[0], shape[1], 2))
...
def _read_spec2(
    ...
    if not numpy_types:
        dtypes = {key: tf.as_dtype(value) for key, value in dtypes.items()}
    shapes = {key: value.shape[1:] for key, value in episode.items()}
    # shapes = {key: (None,) + shape[1:] for key, shape in shapes.items()}
    if return_length:
        length = len(episode[list(shapes.keys())[0]])
        return dtypes, shapes, length
...
def _hard_permuted(sequence, amount, sorted_ind):
    ...
    dia = int(ratio * len(sequence))
    if not sequence:
        return
    index = 0
    while True:
        for element in np.random.permutation(ind):
            if index >= amount:
                return
            ...
            if np.random.random() < 0.5:
                ...
                ubound = min(len(sequence), pos + dia)
                select = np.random.randint(lbound, ubound) if ubound > lbound else element
                rival = sorted_ind[select]
                index += 1
...
def hard_negative_loader(reader, directory, batch_size, every):
    ...
    while True:
        ...
        episodes += _sample(recent.values(), every // 2)
        episodes += _sample(cache.values(), every // 2)
        return_list = [e['return'][-1] for e in episodes]
        indices = [i for i in range(len(episodes))]
        sorted_ind = [e for _, e in sorted(zip(return_list, indices))]
        for episode in _hard_permuted(episodes, every, sorted_ind):
            yield episode
        cache.update(recent)
        recent = {}
        filenames = tf.gfile.Glob(os.path.join(directory, '*.npz'))
